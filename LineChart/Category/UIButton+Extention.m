...
+ (instancetype)btnWithTitle:(NSString *)title rect:(CGRect)rect taget:(id)taget action:(SEL)action
{
    ...
    int btnFontSize = kCommonFontSizeTitle;
...
//    int btnFontSize = kCommonFontSizeTitle + 4;
//    if (isRetina || iPhone5) {
//        btnFontSize = kCommonFontSizeTitle + 2;
//    }
    btn.titleLabel.font = [UIFont systemFontOfSize:kCommonFontSizeTitle];
    [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [btn setTitle:title forState:UIControlStateNormal];
    
    //  设置按钮图标合适位置
    CGSize strSize = [title strSizeWithFont:kCommonFontSizeTitle maxSize:MAX_SIZE];
